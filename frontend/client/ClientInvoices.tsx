...
const EMPTY_DATA: InvoiceData = {
  totalPaid: 0, totalPending: 0, totalOverdue: 0, list: [],
};
...
export function ClientInvoices() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/client/invoices?clientId=${user.id}`)
      ...
      .then((json: InvoiceData) => {
        setData({
          totalPaid:    json.totalPaid    ?? 0,
          totalPending: json.totalPending ?? 0,
          totalOverdue: json.totalOverdue ?? 0,
          list:         Array.isArray(json.list) ? json.list : [],
        });
        setLoading(false);
      })
      .catch((err: Error) => {
        console.error("Client invoices fetch failed:", err.message);
  ...
  return (
    <div className="p-6">
      ...
      {!loading && data.totalOverdue > 0 && (
        <div className="flex items-start gap-2 bg-red-50 border border-red-100 rounded-xl p-3 mb-6 text-sm max-w-lg">
          <AlertCircle size={15} className="text-red-500 mt-0.5 shrink-0" />
          <p className="text-red-700">
            You have <span className="font-semibold">${data.totalOverdue.toLocaleString()}</span> in overdue invoices. Settling these will improve your reliability score.
          </p>
        </div>
      )}
      ...
      {!loading && !error && data.list.length === 0 && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm mt-1">Invoices from your freelancers will appear here.</p>
        </div>
      )}
            ...
            <tbody className="divide-y divide-gray-50">
              {data.list.map((inv) => (
                <tr key={inv.id} className="text-sm hover:bg-gray-50/40 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-600 text-xs">{inv.freelancerName}</td>
