...
public class UserRestController {
    ...
    // Used to auto-send a fresh OTP when an unverified user tries to login
    private final EmailService emailService;

    public UserRestController(EmailService emailService) {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest request) {
        ...
        if (user != null) {
            // Block login if the email hasn't been verified yet.
            // Auto-issue a fresh OTP so the frontend can drop straight into
            // the verification screen without a separate resend click.
            if (!user.isEmailVerified()) {
                System.out.println(">>> LOGIN BLOCKED (unverified): " + request.getEmail());
                try {
                    String otp        = emailService.generateOtp();
                    java.util.Date otpExpires = emailService.computeExpiry();
                    MongoDatabase db  = mongoClient.getDatabase(DB_NAME);
                    db.getCollection("users").updateOne(
                        com.mongodb.client.model.Filters.eq("email", request.getEmail()),
                        Updates.combine(
                    ...
                    System.err.println(">>> Could not auto-resend OTP: " + mailErr.getMessage());
                }

                Map<String, Object> resp = new java.util.HashMap<>();
                resp.put("requiresVerification", true);
                resp.put("email",                request.getEmail());
                resp.put("expiryMinutes",        emailService.getOtpExpiryMinutes());
                resp.put("message",              "Please verify your email. A fresh OTP has been sent.");
